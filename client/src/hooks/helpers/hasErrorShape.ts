 ...
 *
 * Matches on any truthy `error` property, regardless of its type, so a
 * malformed (non-string) error payload still gets caught by callers instead
 * of silently passing through as valid data.
 */
